(function(){
 ...
 window.Malsseum.services.resolveConcernRoles=function(message,analysis){
  ...
  const professionalJudgment=verifiedFacts.some(fact=>fact.type==='medication_decision');
  ...
  const emotionalConcernInHealthContext=explicitCause&&analysis.cause.category==='health'&&analysis.effects.some(effect=>['fatigue','emotional_distress','anxiety'].includes(effect.type)&&isQuoted(message,effect.evidence));
